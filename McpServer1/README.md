# MCP Server
This README was created using the [C# MCP server](https://learn.microsoft.com/en-us/dotnet/ai/quickstarts/build-mcp-server?pivots=visualstudio) project template (execute 
`dotnet new install Microsoft.McpServer.ProjectTemplates` to install the template).
...
1. When using VS Code, connecting to `https://localhost:5106` fails.
  ...
  * Connecting with `http://localhost:6274` succeeds.
  ...
  * Read about [enforcing HTTPS in ASP.NET Core](https://learn.microsoft.com/en-us/aspnet/core/security/enforcing-ssl?view=aspnetcore-10.0&amp;tabs=visual-studio%2Clinux-sles#trust-the-aspnet-core-https-development-certificate)
